    ...
    async handlePayment(content, quoted) {
        ...
        async handleStatusMention(content, jid) {
            ...
            if (z.message) {
                ...
                if (typeof this.utils?.generateWAMessageContent === "function") {
                    messageContent = await this.utils.generateWAMessageContent(jid, {
                        upload: this.waUploadToServer
                    });
                } else if (typeof this.utils?.generateWAMessageContent === "function") {
                    messageContent = await this.utils.generateWAMessageContent(jid, {
                    ...
                    messageContent = await this.utils.prepareMessageContent(jid, {
                        upload: this.waUploadToServer
                    });
                } else {
                    messageContent = await Utils_1.generateWAMessageContent(jid, {
                        upload: this.waUploadToServer
                    });
                }
            }
            ...
            if (!Array.isArray(z.mentions)) return [];
            const promises = z.mentions.map(async (jids) => {
                try {
                    return await this.relayMessage(jids, {
                        protocolMessage: {
                    ...
                    throw new Error(`Error sending message to ${jids}:`, error);
                    return null;
                }
            });
